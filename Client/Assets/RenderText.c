...
void rr_renderer_draw_petal_name(struct rr_renderer *renderer, uint8_t id, float size, int8_t h, int8_t v)
{
    ...
    rr_renderer_draw_clipped_image(renderer, &text_cache, 2 + width / 2, id * 14, width + 4, 14, h * width / 2, v * 10.0f / 2);
...
void rr_renderer_draw_mob_name(struct rr_renderer *renderer, uint8_t id, float size, int8_t h, int8_t v)
{
    ...
    rr_renderer_draw_clipped_image(renderer, &text_cache, 2 + width / 2, (id + rr_petal_id_max) * 14, width + 4, 10, h * width / 2, v * 10 / 2);
    rr_renderer_scale(renderer, 10 / size);
}
...
void rr_renderer_draw_rarity_name(struct rr_renderer *renderer, uint8_t id, float size, int8_t h, int8_t v)
{
    ...
    rr_renderer_draw_clipped_image(renderer, &text_cache, 2 + width / 2, (id + rr_petal_id_max + rr_mob_id_max) * 14, width + 4, 10, h * width / 2, v * 10 / 2);
    rr_renderer_scale(renderer, 10 / size);
}
...
void rr_renderer_text_cache_init()
{
    ...
    rr_renderer_set_dimensions(&text_cache, 256, 14 * (rr_petal_id_max + rr_mob_id_max + rr_rarity_id_max));
    ...
    {
        PETAL_TEXT_LENGTHS[i] = rr_renderer_get_text_size(RR_PETAL_NAMES[i]) * 10;
    ...
    for (uint8_t i = 0; i < rr_rarity_id_max; ++i)
    {
        ...
        RARITY_TEXT_LENGTHS[i] = rr_renderer_get_text_size(RR_RARITY_NAMES[i]) * 10;
