...
static void v_slider_on_render(struct rr_ui_element *this, struct rr_game *game)
{
    struct rr_renderer *renderer = game->renderer;
    struct rr_ui_slider_metadata *data = this->data;
    ...
    rr_renderer_begin_path(renderer);
    rr_renderer_move_to(renderer, 0, -(this->abs_width - this->abs_height) / 2);
    rr_renderer_line_to(renderer, 0, (this->abs_width - this->abs_height) / 2);
    rr_renderer_stroke(renderer);
    float offset = (this->abs_width - this->abs_height) * (*data->value - 0.5);
    rr_renderer_begin_path(renderer);
    rr_renderer_arc(renderer, 0, offset, this->abs_width / 2);
    rr_renderer_fill(renderer);
    if (rr_ui_mouse_over(this, game) &&
        game->input_data->mouse_buttons_down_this_tick & 1)
    {
