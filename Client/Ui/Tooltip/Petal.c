...
struct rr_ui_element *rr_ui_petal_tooltip_init(uint8_t id, uint8_t rarity)
{
    ...
    {
        char *extra = malloc((sizeof *extra) * 8);
        extra[sprintf(extra, "%.1f/s", 25 * 0.05 * RR_PETAL_RARITY_SCALE[rarity].damage)] = 0;
        rr_ui_container_add_element(this, rr_ui_set_justify(rr_ui_h_container_init(
                rr_ui_container_init(), 0, 0,
    ...
    else if (id == rr_petal_id_egg && rarity > 0)
    {
        rr_ui_container_add_element(this, rr_ui_set_justify(rr_ui_h_container_init(
                ...
                rr_ui_text_init(RR_RARITY_NAMES[rarity - 1], 12, RR_RARITY_COLORS[rarity - 1]),
    ...
    {
        char *extra = malloc((sizeof *extra) * 8);
        extra[sprintf(extra, "%.0f", 40 + RR_PETAL_RARITY_SCALE[rarity].damage * 30)] = 0;
        rr_ui_container_add_element(this, rr_ui_set_justify(rr_ui_h_container_init(
                rr_ui_container_init(), 0, 0,
