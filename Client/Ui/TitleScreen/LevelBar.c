...
{   
    ...
    else
    {
        ...
        {
            data->lerp_xp = rr_lerp(data->lerp_xp, xp, 0.1);
        }
        else
        {
            data->lerp_xp = rr_lerp(data->lerp_xp, xp_to_reach_level(data->level + 1), 0.1);
            if (data->lerp_xp >= xp_to_reach_level(data->level + 1) * 0.999)
            {
                ++data->level;
                data->lerp_xp = 0;
            }
