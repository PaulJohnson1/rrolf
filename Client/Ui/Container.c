...
static void container_2d_on_render(struct rr_ui_element *this,
                                   struct rr_game *game)
{
                                                  ...
                                                  struct rr_ui_element *add)
{
    ...
    if (++data->size == data->capacity)
    {
        struct rr_ui_element **_new =
            malloc((sizeof *_new) * (data->capacity *= 2));
...
struct rr_ui_element *rr_ui_flex_container_init()
{
    ...
    this->poll_events = container_poll_events;
    this->resizeable =
        1; // shows that it is resizeable and fits the size of its container
    return this;
}
