...
static void strinject(char *bufstart, char *seq, uint32_t utf8pos, uint32_t max)
{
    ...
    memcpy(temp + (reader.at - reader.start) + injsize, reader.at, max - (reader.at - reader.start));
...
static void strremv(char *bufstart, uint32_t utf8pos, uint32_t rem_len, uint32_t max)
{
    if (rem_len > utf8pos)
        rem_len = utf8pos;
    struct rr_binary_encoder reader;
...
static void text_input_on_event(struct rr_ui_element *this, struct rr_game *game)
{
...
static void text_input_on_render(struct rr_ui_element *this, struct rr_game *game)
{
    ...
    if ((game->input_data->mouse_buttons_down_this_tick & 1) && data->focused) {
        ...
        char buf[5];
        for (data->caret_pos = 0; data->caret_pos < strlen(data->buffer); ++data->caret_pos)
        {
            ...
            diff -= rr_renderer_get_text_size(buf) * this->abs_height * renderer->scale;
            if (diff < 0)
                break;
        }
    }
    rr_renderer_begin_path(renderer);
    rr_renderer_rect(renderer, -this->width / 2, -this->height / 2,
    ...
    *encoder.at = 0;
    float text_length = rr_renderer_get_text_size(data->buffer) * this->abs_height;
    data->relative_x = rr_fclamp(this->abs_width * 0.96 - text_length, -1000, 0);
    *encoder.at = repl;
    rr_renderer_translate(renderer, data->relative_x - this->abs_width * 0.48, 0); //positive is left
    ...
    if (game->is_mobile)
    {
        if (rr_ui_mouse_over(this, game) && (game->input_data->mouse_buttons_down_this_tick & 1))
        {
            EM_ASM({
                const str = prompt();
                const arr = new TextEncoder().encode(str+'/x00').slice(0,$1);
                HEAPU8.set(arr, $0);
...
struct rr_ui_element *rr__ui_text_input_init(float w, float h, char *text, uint8_t max_length, char *name)
{
