...
static void loadout_button_on_event(struct rr_ui_element *this, struct rr_game *game)
{
    ...
    else
    {
        if (game->loadout[data->pos].id == 0)
            return;
        struct rr_ui_element *tooltip = game->petal_tooltips[data->prev_id][data->prev_rarity];
...
static void petal_switch_button_event(struct rr_ui_element *this, struct rr_game *game)
{
    ...
    else
    {
        ...
        struct rr_component_player_info_petal_slot *slot = data->pos < 10 ? &player_info->slots[data->pos] : &player_info->secondary_slots[data->pos - 10];
        uint8_t id = slot->id;
...
static uint8_t title_screen_loadout_button_animate(struct rr_ui_element *this, struct rr_game *game)
{
    struct loadout_button_metadata *data = this->data;
...
static uint8_t loadout_button_animate(struct rr_ui_element *this, struct rr_game *game)
{
...
static void loadout_button_on_render(struct rr_ui_element *this, struct rr_game *game)
{
    struct loadout_button_metadata *data = this->data;
    struct rr_renderer *renderer = game->renderer;
