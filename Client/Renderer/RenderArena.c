...
void render_background(struct rr_component_player_info *player_info, struct rr_game *this)
{
    ...
    double leftX =
        player_info->lerp_camera_x - renderer->width / (2 * scale);
    double rightX =
        player_info->lerp_camera_x + renderer->width / (2 * scale);
    double topY =
    ...
    for (; nx < rightX / grid_size; ++nx)
        for (int32_t currY = ny; currY < bottomY / grid_size; ++currY)
        {
            ...
            if (tile != 1)
            {
                ...
                if (tile == 0)
                    rr_renderer_fill_rect(renderer, nx * grid_size, currY * grid_size, grid_size, grid_size);
                else
                {
                    uint8_t left = (tile >> 1) & 1;
                    uint8_t top = tile & 1;
                    uint8_t inverse = 1 - ((tile >> 3) & 1);
                    rr_renderer_move_to(renderer, (nx + inverse ^ left) * grid_size, (currY + inverse ^ top) * grid_size);
                    float start_angle = 0;
                    if (top == 0 && left == 1)
                        start_angle = M_PI / 2;
