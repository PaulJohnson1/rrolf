...
void rr_component_petal_render(EntityIdx entity, struct rr_game *game, struct rr_simulation *simulation)
{
    ...
    if (game->cache.screen_shake)
        rr_renderer_add_color_filter(renderer, RR_RARITY_COLORS[petal->rarity], 0.25);
    ...
    uint8_t use_cache = (((health->damage_animation < 0.1) | game->cache.low_performance_mode) & 1) & (1 - game->cache.screen_shake);
