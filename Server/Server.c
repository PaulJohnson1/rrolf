...
static void rr_server_client_create_player_info(struct rr_server *server, struct rr_server_client *client)
{
    ...
    struct rr_simulation *simulation = rr_client_get_simulation(server, client);
    ...
    struct rr_component_arena *arena =
        rr_simulation_get_arena(simulation, 1);
        ...
        case RR_SERVERBOUND_PETAL_SWITCH:
        {
                   ...
                   encoder.current - encoder.start <= size)
            {
                rr_component_player_info_petal_swap(client->player_info,
                                                    rr_client_get_simulation(this, client), pos - 1);
...
void rr_squad_tick(struct rr_squad *squad, struct rr_server *server)
{
    if (squad->member_count == 0 && squad->simulation_active)
    {
        squad->simulation.game_over = 1;
        squad->simulation_active = 0;
    }
    if (squad->member_count == 0)
        return;
    if (squad->simulation_active)
        rr_simulation_tick(&squad->simulation);
    else
    {
        uint8_t allow = 1;
        ...
        if (squad->members[i].in_use)
        {
            ...
            else
            {
                ...
                proto_bug_write_uint8(&encoder, client->squad_pos, "sqpos");
                    ...
                    proto_bug_write_uint8(&encoder, server->biome, "biome");
                    char joined_code[16];
                    joined_code[sprintf(joined_code, "%s-%s", server->server_alias, squad->squad_code)] = 0;
    ...
    if (squad->simulation_active)
        rr_simulation_for_each_entity(
            &squad->simulation, &squad->simulation,
