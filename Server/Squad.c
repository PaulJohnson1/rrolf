...
void rr_client_leave_squad(struct rr_server *this, struct rr_server_client *member)
{
    if (member->squad == 0)
        return;
    fprintf(stderr, "leaving squad %d\n", member->squad - 1);
...
struct rr_simulation *rr_client_get_simulation(struct rr_server *this, struct rr_server_client *member)
{
    if (member->squad == 0)
        return NULL;
    return &this->squads[member->squad - 1].simulation;
}