...
static void system_petal_detach(struct rr_simulation *simulation, struct rr_component_petal *petal, struct rr_component_player_info *player_info,
                                uint32_t outer_pos, uint32_t inner_pos, struct rr_petal_data const *petal_data)
{
    petal->detached = 1;
    struct rr_component_player_info_petal *ppetal = &player_info->slots[outer_pos].petals[inner_pos];
    ppetal->simulation_id = RR_NULL_ENTITY;
    ppetal->cooldown_ticks = petal_data->cooldown;
}
// sinc ethis is static, the fucntion name doesn't need any sorto f prefix and can just be petal_logic
static void system_flower_petal_movement_logic(struct rr_simulation *simulation, EntityIdx id, struct rr_component_player_info *player_info,
                                               uint32_t rotation_pos, uint32_t outer_pos, uint32_t inner_pos, struct rr_petal_data const *petal_data)
{
...
static void rr_system_petal_reload_foreach_function(EntityIdx id, void *simulation)
{
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
        ...
        for (uint64_t inner = 0; inner < slot->count; ++inner)
        {
            ...
            if (p_petal->simulation_id != RR_NULL_ENTITY && !rr_simulation_has_entity(simulation, p_petal->simulation_id))
            {
            ...
            if (p_petal->simulation_id == RR_NULL_ENTITY)
            {
                if (--p_petal->cooldown_ticks <= 0)
                {
                    ...
                    rr_component_health_set_max_health(health, scale * data->health);
                    rr_component_health_set_health(health, scale * data->health);
...
static void system_non_player_owned_petal_movement_logic(EntityIdx id, void *simulation)
{
    struct rr_component_petal *petal = rr_simulation_get_petal(simulation, id);
    struct rr_component_physical *physical = rr_simulation_get_physical(simulation, id);
...
{
    rr_simulation_for_each_player_info(simulation, simulation, rr_system_petal_reload_foreach_function);
    rr_simulation_for_each_petal(simulation, simulation, system_non_player_owned_petal_movement_logic);
}
