                                 ...
                                 struct rr_component_petal *petal)
{
    if (--petal->effect_delay == 0)
    {
        ...
        if (flower_physical->has_deletion_animation == 0)
            return;
            ...
            else
            {
                ...
                if (data->id == rr_petal_id_egg)
                {
                    ...
                    EntityIdx mob_id = p_petal->simulation_id =
                        rr_simulation_alloc_mob(simulation, petal_physical->x, petal_physical->y, petal->rarity ? rr_mob_id_trex : rr_mob_id_meteor,
                                                petal->rarity ? petal->rarity - 1 : 0,
                                                rr_simulation_team_id_players);
                    struct rr_component_relations *relations =
    ...
    else
    {
            ...
            0)
        {
            ...
            if (petal->id == rr_petal_id_seed &&
                physical->has_deletion_animation)
            {
