...
#define spatial_hash_get(x,y) &this->cells[(x)][(y)]
...
void rr_spatial_hash_insert(struct rr_spatial_hash *this, EntityIdx entity)
{
    ...
    uint32_t x = rr_fclamp(physical->x+RR_ARENA_RADIUS, 0, 2 * RR_ARENA_RADIUS) / SPATIAL_HASH_GRID_SIZE;
    uint32_t y = rr_fclamp(physical->y+RR_ARENA_RADIUS, 0, 2 * RR_ARENA_RADIUS) / SPATIAL_HASH_GRID_SIZE;
...
{
    float x = fmaxf(fx - fw + RR_ARENA_RADIUS, 0);
    float y = fmaxf(fy - fh + RR_ARENA_RADIUS, 0);
